...
define('tinymce/inlite/ui/Toolbar', [
	...
	var setActiveItem = function (item, name) {
		return function(state, args) {
			...
			while (i--) {
				nodeName = args.parents.nodeName;
	...
	var emptyTextBlock = function (elements, id) {
		return function (editor) {
			...
			for (i = 0; i < elements.length; i++) {
				if (elements.nodeName === 'TABLE') {
					return null;
				}
			}

			for (i = 0; i < elements.length; i++) {
				if (elements.nodeName in textBlockElementsMap) {
					if (editor.dom.isEmpty(elements)) {
						return Matcher.result(id, Measure.getSelectionRect(editor));
					}
	...
	var element = function (element, predicateIds) {
		return function (editor) {
			for (var i = 0; i < predicateIds.length; i++) {
				if (predicateIds.predicate(element)) {
					return Matcher.result(predicateIds.id, Measure.getElementRect(editor, element));
				}
			}
		...
		return function (editor) {
			for (var i = 0; i < elements.length; i++) {
				for (var x = 0; x < predicateIds.length; x++) {
					if (predicateIds[x].predicate(elements)) {
						return Matcher.result(predicateIds[x].id, Measure.getElementRect(editor, elements));
					}
				}
			}
