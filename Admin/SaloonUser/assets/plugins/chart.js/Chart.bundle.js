...
function getRgba(string) {
   ...
   if (match) {
      ...
      for (var i = 0; i < rgb.length; i++) {
         rgb = parseInt(match + match, 16);
      }
   }
   else if (match = string.match(hex)) {
      match = match[1];
      for (var i = 0; i < rgb.length; i++) {
         rgb = parseInt(match.slice(i * 2, i * 2 + 2), 16);
      }
   }
   else if (match = string.match(rgba)) {
      for (var i = 0; i < rgb.length; i++) {
         rgb = parseInt(match[i + 1]);
      }
      a = parseFloat(match[4]);
   }
   else if (match = string.match(per)) {
      for (var i = 0; i < rgb.length; i++) {
         rgb = Math.round(parseFloat(match[i + 1]) * 2.55);
      }
      a = parseFloat(match[4]);
   }
...
Color.prototype = {
	...
	luminosity: function () {
		...
		for (var i = 0; i < rgb.length; i++) {
			var chan = rgb / 255;
			lum = (chan <= 0.03928) ? chan / 12.92 : Math.pow(((chan + 0.055) / 1.055), 2.4);
		}
...
Color.prototype.getValues = function (space) {
	...
	for (var i = 0; i < space.length; i++) {
		vals[space.charAt(i)] = values[space];
	}
...
Color.prototype.setValues = function (space, vals) {
	...
	if (space === 'alpha') {
		...
		for (i = 0; i < space.length; i++) {
			values[space] = vals[space.charAt(i)];
		}
	...
	for (i = 0; i < space.length; i++) {
		capped = Math.max(0, Math.min(maxes[space], values[space]));
		values[space] = Math.round(capped);
	}
...
function compareArrays(array1, array2, dontConvert) {
    var len = Math.min(array1.length, array2.length),
        ...
        i;
    for (i = 0; i < len; i++) {
        if ((dontConvert && array1 !== array2) ||
            (!dontConvert && toInt(array1) !== toInt(array2))) {
            diffs++;
        }
    }
...
function stringSet (units, value) {
    if (typeof units === 'object') {
        ...
        for (var i = 0; i < prioritized.length; i++) {
            this[prioritized.unit](units[prioritized.unit]);
        }
    } else {
        units = normalizeUnits(units);
        if (isFunction(this[units])) {
...
function makeFormatFunction(format) {
    ...
    for (i = 0, length = array.length; i < length; i++) {
        if (formatTokenFunctions[array]) {
            array = formatTokenFunctions[array];
        } else {
            array = removeFormattingTokens(array);
        }
    }
    ...
    return function (mom) {
        var output = '', i;
        for (i = 0; i < length; i++) {
            output += isFunction(array) ? array.call(mom, format) : array;
        }
        return output;
    };
}
...
function handleStrictParse(monthName, format, strict) {
    ...
    if (!this._monthsParse) {
        ...
        for (i = 0; i < 12; ++i) {
            ...
            this._shortMonthsParse = this.monthsShort(mom, '').toLocaleLowerCase();
            this._longMonthsParse = this.months(mom, '').toLocaleLowerCase();
        }
    }
...
function localeMonthsParse (monthName, format, strict) {
    ...
    for (i = 0; i < 12; i++) {
        ...
        if (strict && !this._longMonthsParse) {
            this._longMonthsParse = new RegExp('^' + this.months(mom, '').replace('.', '') + '$', 'i');
            this._shortMonthsParse = new RegExp('^' + this.monthsShort(mom, '').replace('.', '') + '$', 'i');
        }
        if (!strict && !this._monthsParse) {
            regex = '^' + this.months(mom, '') + '|^' + this.monthsShort(mom, '');
            this._monthsParse = new RegExp(regex.replace('.', ''), 'i');
        }
        // test the regex
        if (strict && format === 'MMMM' && this._longMonthsParse.test(monthName)) {
            return i;
        } else if (strict && format === 'MMM' && this._shortMonthsParse.test(monthName)) {
            return i;
        } else if (!strict && this._monthsParse.test(monthName)) {
            return i;
        }
    }
}
...
function computeMonthsParse () {
    ...
    for (i = 0; i < 12; i++) {
        shortPieces = regexEscape(shortPieces);
        longPieces = regexEscape(longPieces);
    }
    for (i = 0; i < 24; i++) {
        mixedPieces = regexEscape(mixedPieces);
    }
...
function handleStrictParse$1(weekdayName, format, strict) {
    ...
    if (!this._weekdaysParse) {
        ...
        for (i = 0; i < 7; ++i) {
            ...
            this._minWeekdaysParse = this.weekdaysMin(mom, '').toLocaleLowerCase();
            this._shortWeekdaysParse = this.weekdaysShort(mom, '').toLocaleLowerCase();
            this._weekdaysParse = this.weekdays(mom, '').toLocaleLowerCase();
        }
    }
...
function localeWeekdaysParse (weekdayName, format, strict) {
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (strict && !this._fullWeekdaysParse) {
            this._fullWeekdaysParse = new RegExp('^' + this.weekdays(mom, '').replace('.', '\.?') + '$', 'i');
            this._shortWeekdaysParse = new RegExp('^' + this.weekdaysShort(mom, '').replace('.', '\.?') + '$', 'i');
            this._minWeekdaysParse = new RegExp('^' + this.weekdaysMin(mom, '').replace('.', '\.?') + '$', 'i');
        }
        if (!this._weekdaysParse) {
            regex = '^' + this.weekdays(mom, '') + '|^' + this.weekdaysShort(mom, '') + '|^' + this.weekdaysMin(mom, '');
            this._weekdaysParse = new RegExp(regex.replace('.', ''), 'i');
        }
        // test the regex
        if (strict && format === 'dddd' && this._fullWeekdaysParse.test(weekdayName)) {
            return i;
        } else if (strict && format === 'ddd' && this._shortWeekdaysParse.test(weekdayName)) {
            return i;
        } else if (strict && format === 'dd' && this._minWeekdaysParse.test(weekdayName)) {
            return i;
        } else if (!strict && this._weekdaysParse.test(weekdayName)) {
            return i;
        }
    }
}
...
function chooseLocale(names) {
    ...
    while (i < names.length) {
        split = normalizeLocale(names).split('-');
        j = split.length;
...
function configFromISO(config) {
    ...
    if (match) {
        ...
        for (i = 0, l = isoDates.length; i < l; i++) {
            if (isoDates[1].exec(match[1])) {
                dateFormat = isoDates[0];
                allowTime = isoDates[2] !== false;
                break;
            }
        }
        if (dateFormat == null) {
        ...
        if (match[3]) {
            for (i = 0, l = isoTimes.length; i < l; i++) {
                if (isoTimes[1].exec(match[3])) {
                    // match[2] should be 'T' or space
                    timeFormat = (match[2] || ' ') + isoTimes[0];
                    break;
                }
            }
            if (timeFormat == null) {
...
function configFromArray (config) {
    ...
    for (i = 0; i < 3 && config._a == null; ++i) {
        config._a = input = currentDate;
    }

    // Zero out whatever was not defaulted, including time
    for (; i < 7; i++) {
        config._a = input = (config._a == null) ? (i === 2 ? 1 : 0) : config._a;
    }
...
function pickBy(fn, moments) {
    ...
    for (i = 1; i < moments.length; ++i) {
        if (!moments.isValid() || moments[fn](res)) {
            res = moments;
        }
    }
    return res;
...
function isDurationValid(m) {
    ...
    for (var i = 0; i < ordering.length; ++i) {
        if (m[ordering]) {
            ...
            if (parseFloat(m[ordering]) !== toInt(m[ordering])) {
                unitHasDecimal = true;
            }
        }
    }
...
function listMonthsImpl (format, index, field) {
    ...
    var out = [];
    for (i = 0; i < 12; i++) {
        out = get$1(format, i, field, 'month');
    }
    return out;
}
...
function listWeekdaysImpl (localeSorted, format, index, field) {
    ...
    var out = [];
    for (i = 0; i < 7; i++) {
        out = get$1(format, (i + shift) % 7, field, 'day');
    }
    return out;
}
	...
	Chart.controllers.bar = Chart.DatasetController.extend({
		...
		update: function(reset) {
			...
			for (i = 0, ilen = elements.length; i < ilen; ++i) {
				me.updateElement(elements, i, reset);
			}
		},
			...
			if (stacked || (stacked === undefined && stack !== undefined)) {
				for (i = 0; i < datasetIndex; ++i) {
					...
					if (imeta.bar &&
						...
						ivalue = Number(datasets.data[index]);
		...
		legendCallback: function(chart) {
			...
			if (datasets.length) {
				for (var i = 0; i < datasets[0].data.length; ++i) {
					text.push('<li><span style="background-color:' + datasets[0].backgroundColor + '"></span>');
					if (labels) {
						text.push(labels);
					}
					text.push('</li>');
					...
					if (data.labels.length && data.datasets.length) {
						return data.labels.map(function(label, i) {
							...
							return {
								...
								strokeStyle: stroke,
								lineWidth: bw,
								hidden: isNaN(ds.data) || meta.data.hidden,
	...
	Chart.controllers.doughnut = Chart.controllers.pie = Chart.DatasetController.extend({
		...
		getMaxBorderWidth: function(elements) {
			...
			for (var i = 0; i < length; i++) {
				borderWidth = elements._model ? elements._model.borderWidth : 0;
				hoverWidth = elements._chart ? elements._chart.config.data.datasets[index].hoverBorderWidth : 0;
	...
	Chart.controllers.line = Chart.DatasetController.extend({
		...
		update: function(reset) {
			...
			for (i=0, ilen=points.length; i<ilen; ++i) {
				me.updateElement(points, i, reset);
			}
	...
	Chart.controllers.polarArea = Chart.DatasetController.extend({
		...
		updateElement: function(arc, index, reset) {
			...
			for (var i = 0; i < index; ++i) {
				if (!isNaN(dataset.data) && !meta.data.hidden) {
					++visibleCount;
				}
			}
	...
	helpers.extend(Chart.DatasetController.prototype, {
		...
		addElements: function() {
			...
			for (i=0, ilen=data.length; i<ilen; ++i) {
				metaData = metaData || me.createMetaData(i);
			}
		...
		transition: function(easingValue) {
			...
			for (; i<ilen; ++i) {
				elements.transition(easingValue);
			}
...
module.exports = function(Chart) {
	...
	helpers.each = function(loopable, callback, self, reverse) {
		...
		if (helpers.isArray(loopable)) {
			...
			len = keys.length;
			for (i = 0; i < len; i++) {
				callback.call(self, loopable[keys], keys);
			}
		}
	};
	...
	helpers.findIndex = Array.prototype.findIndex?
		...
		function(array, callback, scope) {
			...
			for (var i = 0, ilen = array.length; i < ilen; ++i) {
				if (callback.call(scope, array, i, array)) {
					return i;
				}
			}
			return -1;
		...
		global: {
			...
			legendCallback: function(chart) {
				...
				for (var i = 0; i < chart.data.datasets.length; i++) {
					text.push('<li><span style="background-color:' + chart.data.datasets.backgroundColor + '"></span>');
					if (chart.data.datasets.label) {
						text.push(chart.data.datasets.label);
					}
					text.push('</li>');
				}
				text.push('</ul>');
		...
		update: function(changed) {
			...
			if (active.length) {
				...
				for (i = 0, len = active.length; i < len; ++i) {
					tooltipItems.push(createTooltipItem(active));
				}
...
module.exports = function(Chart) {
	...
	function drawArea(ctx, curve0, curve1, len0, len1) {
		...
		for (i=1; i<len0; ++i) {
			helpers.canvas.lineTo(ctx, curve0[i-1], curve0);
		}
		...
		for (i=len1-1; i>0; --i) {
			helpers.canvas.lineTo(ctx, curve1, curve1[i-1], true);
		}
	}
				...
				for (var i = 0; i < lh.length; ++i) {
					...
					if (x >= hitBox.left && x <= hitBox.left + hitBox.width && y >= hitBox.top && y <= hitBox.top + hitBox.height) {
						...
						if (type === 'click') {
							...
							opts.onClick.call(me, e.native, me.legendItems);
							changed = true;
							break;
						} else if (type === 'mousemove') {
							// use e.native for backwards compatibility
							opts.onHover.call(me, e.native, me.legendItems);
							changed = true;
							break;
						}
					}
...
module.exports = function(Chart) {
	...
	function fitWithPointLabels(scale) {
		...
		for (i = 0; i < valueCount; i++) {
			...
			textSize = measureLabelSize(scale.ctx, plFont.size, scale.pointLabels || '');
			scale._pointLabelSizes = textSize;
	...
	function drawPointLabels(scale) {
		...
		for (var i = getValueCount(scale) - 1; i >= 0; i--) {
			...
			if (pointLabelOpts.display) {
				...
				adjustPointPositionForLabelHeight(angle, scale._pointLabelSizes, pointLabelPosition);
				fillText(ctx, scale.pointLabels || '', pointLabelPosition, plFont.size);
			}
		}
	}
	...
	function determineStepSize(min, max, unit, maxTicks) {
		...
		if (unitDefinition.steps) {
			...
			for (var i = 0; i < numSteps && sizeInUnits > maxTicks; i++) {
				multiplier = unitDefinition.steps;
